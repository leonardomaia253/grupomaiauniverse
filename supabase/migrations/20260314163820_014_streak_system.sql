...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('checked_in', false, 'error', 'company_not_found');
  END IF;

  IF v_last_date = v_today THEN
    RETURN jsonb_build_object('checked_in', false, 'already_today', true, 'streak', v_streak, 'longest', v_longest);
  END IF;
  ...
  RETURN jsonb_build_object('checked_in', true, 'already_today', false, 'streak', v_streak, 'longest', v_longest, 'was_frozen', v_was_frozen);
END;
$$;
