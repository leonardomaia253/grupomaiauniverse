...
create index idx_purchases_dev on purchases(company_id, status);
...
create unique index idx_purchases_unique_completed
  on purchases(company_id, item_id) where status = 'completed';

-- Company customizations (config per item, e.g. color choice)
...
create policy "Public read customizations" on company_customizations for select using (true);

-- Seed: item catalog
insert into items (id, category, name, description, price_usd_cents, price_brl_cents, metadata) values
  ('neon_outline',    'effect',    'Neon Outline',    'Glowing outline on planet edges',         200, 990, '{}'),
  ('particle_aura',   'effect',    'Particle Aura',   'Floating particles around the planet',    300, 1490, '{}'),
  ...
  ('custom_color',    'identity',  'Custom Color',    'Choose your planet color',                150, 790, '{"default_color": "#c8e64a"}'),
  ('billboard',       'identity',  'Billboard',       'Logo or image on the planet side',        300, 1490, '{}'),
