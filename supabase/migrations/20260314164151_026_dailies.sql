...
CREATE INDEX IF NOT EXISTS idx_dmp_dev_date ON daily_mission_progress(company_id, mission_date DESC);
...
CREATE POLICY "dmp_service_insert" ON daily_mission_progress FOR INSERT WITH CHECK (false);
CREATE POLICY "dmp_service_update" ON daily_mission_progress FOR UPDATE USING (false);
...
CREATE OR REPLACE FUNCTION record_mission_progress(p_company_id bigint, p_mission_id text, p_threshold int, p_increment int DEFAULT 1)
RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE v_today date := current_date; v_progress int; v_completed boolean;
BEGIN
  ...
  WHERE company_id = p_company_id AND mission_date = v_today AND mission_id = p_mission_id;

  IF v_progress >= p_threshold AND NOT v_completed THEN
    UPDATE daily_mission_progress SET completed = true, completed_at = now()
    WHERE company_id = p_company_id AND mission_date = v_today AND mission_id = p_mission_id;
    v_completed := true;
  END IF;

  RETURN jsonb_build_object('progress', v_progress, 'completed', v_completed, 'threshold', p_threshold);
END;
$$;

CREATE OR REPLACE FUNCTION complete_all_dailies(p_company_id bigint)
RETURNS jsonb LANGUAGE plpgsql AS $$
DECLARE v_today date := current_date; v_last_date date; v_old_streak int; v_new_streak int; v_total int;
BEGIN
  SELECT last_dailies_date, dailies_streak, dailies_completed INTO v_last_date, v_old_streak, v_total
  FROM companies WHERE id = p_company_id FOR UPDATE;
  IF v_last_date = v_today THEN RETURN jsonb_build_object('already_completed', true, 'streak', v_old_streak, 'total', v_total); END IF;
  v_new_streak := CASE WHEN v_last_date = v_today - 1 THEN v_old_streak + 1 ELSE 1 END;
  v_total := v_total + 1;
  UPDATE companies SET dailies_completed = v_total, dailies_streak = v_new_streak, last_dailies_date = v_today WHERE id = p_company_id;
  RETURN jsonb_build_object('already_completed', false, 'streak', v_new_streak, 'total', v_total);
END;
$$;
