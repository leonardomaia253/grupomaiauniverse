...
BEGIN
  UPDATE companies SET current_week_kudos_given = current_week_kudos_given + 1 WHERE id = p_giver_id;
  UPDATE companies SET current_week_kudos_received = current_week_kudos_received + 1 WHERE id = p_receiver_id;
END;
$$;
...
BEGIN
  ...
  FROM (SELECT giver_id, COUNT(*) as cnt FROM company_kudos WHERE given_date >= week_start GROUP BY giver_id) sub
  WHERE d.id = sub.giver_id;

  UPDATE companies d SET current_week_kudos_received = COALESCE(sub.cnt, 0)
  FROM (SELECT receiver_id, COUNT(*) as cnt FROM company_kudos WHERE given_date >= week_start GROUP BY receiver_id) sub
  WHERE d.id = sub.receiver_id;

  UPDATE companies SET current_week_kudos_given = 0
  WHERE current_week_kudos_given > 0
  AND id NOT IN (SELECT giver_id FROM company_kudos WHERE given_date >= week_start);

  UPDATE companies SET current_week_kudos_received = 0
  WHERE current_week_kudos_received > 0
  AND id NOT IN (SELECT receiver_id FROM company_kudos WHERE given_date >= week_start);
END;
$$;;
