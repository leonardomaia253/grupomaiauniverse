...
CREATE POLICY "Anyone can read snapshot cache" ON universe_snapshot_cache FOR SELECT USING (true);

CREATE OR REPLACE FUNCTION refresh_universe_snapshot()
RETURNS void LANGUAGE plpgsql SET statement_timeout = '180s' AS $$
DECLARE snapshot json;
BEGIN
...
RETURNS jsonb LANGUAGE sql STABLE SET statement_timeout = '5s' AS $$
  SELECT data FROM universe_snapshot_cache WHERE id = 1;
$$;

CREATE OR REPLACE FUNCTION get_universe_snapshot()
RETURNS json LANGUAGE sql STABLE SET statement_timeout = '180s' AS $$
...
DO $$ BEGIN PERFORM cron.unschedule('refresh-city-snapshot'); EXCEPTION WHEN OTHERS THEN END; $$;
SELECT cron.schedule('refresh-city-snapshot', '*/5 * * * *', $$SELECT refresh_universe_snapshot()$$);;
