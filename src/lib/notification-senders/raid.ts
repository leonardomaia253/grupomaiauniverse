...
export function sendRaidAlertNotification(
  ...
  const outcome = success
    ? `@${attackerLogin} attacked your planet!`
    : `You defended against @${attackerLogin}!`;

  const labelColor = success ? "#cc4444" : "#5a8a00";
  const label = success ? "planet attacked" : "Defense successful";
  ...
  sendNotificationAsync({
    ...
    html: `
      ...
      ${buildButton("View Your planet", `${BASE_URL}/?user=${defenderLogin}`)}
    `,
