...
// Simple in-memory cache for company emails (per-process, resets on cold start)
const emailCache = new Map<number, { email: string | null; ts: number }>();
const CACHE_TTL = 60_000; // 1 minute

/**
 * Get a company's email, with fallback to auth.users via admin API.
 * Returns null if no email found (e.g. private GitHub email).
 */
export async function getcompanyEmail(devId: number): Promise<string | null> {
...
/**
 * Check if a company was active in the last N minutes.
...
/**
 * Get multiple companies' push tokens for batch sends.
 */
...
/**
 * Update last_active_at for a company. Fire-and-forget.
 */
...
/**
 * Cache email on companies table from auth callback.
 */
...
/**
 * Ensure notification_preferences row exists for a company.
 */
export async function ensurePreferences(devId: number): Promise<void> {
  const sb = getSupabaseAdmin();
  await sb
    .from("notification_preferences")
    .upsert({ company_id: devId }, { onConflict: "company_id", ignoreDuplicates: true });
}
