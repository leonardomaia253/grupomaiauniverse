...
const MISSION_POOL: Mission[] = [
  ...
  { id: "give_kudos_3",       title: "Kudos spree",        description: "Give kudos to 3 companies",         threshold: 3 },
  { id: "visit_planet",     title: "planet inspector",description: "Visit a dev's planet",        threshold: 1 },
  { id: "visit_3_planets",  title: "Universe explorer",      description: "Visit 3 planets",            threshold: 3 },
  ...
  { id: "explore_constellation",   title: "constellation hopper",    description: "Explore a different constellation",  threshold: 1 },
];
...
/**
 * Get the 3 daily missions for a company on a given date.
 * Deterministic: same date + companyId always gives same missions.
...
export function getDailyMissions(
  ...
  const seed = hashStr(`${dateStr}:${companyId}`);
...
export async function trackDailyMission(
  ...
  try {
    ...
    const missions = getDailyMissions(companyId, today);
