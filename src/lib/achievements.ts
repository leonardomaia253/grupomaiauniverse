...
/**
 * Check and unlock new achievements for a company.
...
export async function checkAchievements(
  ...
  // Batch insert company_achievements
  ...
  await sb
    .from("company_achievements")
    .upsert(unlockRows, { onConflict: "company_id,achievement_id" });
  ...
  if (itemRewards.length > 0) {
    const purchaseRows = itemRewards.map((a) => ({
      ...
      provider_tx_id: `achievement_${companyId}_${a.id}`,
      amount_cents: 0,
      currency: "usd",
      status: "completed",
    }));

    // Batch upsert — unique index on (company_id, item_id) prevents duplicates
    await sb
      .from("purchases")
      .upsert(purchaseRows, { onConflict: "company_id,item_id" });
  }
  ...
  // Notify company of gold/diamond achievements (fire-and-forget)
  if (actorLogin) {
    void (async () => {
      try {
...
/**
 * Batch fetch achievements for multiple companies (for Universe API).
 * Automatically chunks large ID arrays to stay within Supabase query limits.
 */
export async function getAchievementsForcompanies(
  companyIds: number[]
  ...
  for (const row of rows) {
    ...
    result[row.company_id].push(row.achievement_id);
  }
  return result;
}
