...
export async function sendNotification(payload: NotificationPayload): Promise<SendResult[]> {
  ...
  if (payload.skipIfActive) {
    if (await isRecentlyActive(payload.companyId)) {
  ...
  const prefs = await getPreferences(payload.companyId);
  ...
  for (const channel of targetChannels) {
    try {
      ...
      console.error(`[notify] ${channel} error for dev ${payload.companyId}:`, err);
...
export function sendNotificationAsync(payload: NotificationPayload): void {
  sendNotification(payload).catch((err) => {
    console.error(`[notify:async] Failed for ${payload.type} dev=${payload.companyId}:`, err);
  });
}
...
export async function flushPendingBatches(): Promise<number> {
  ...
  const { data: batches } = await sb
    ...
    .select("id, batch_key, company_id, notification_type, channel")
...
async function processChannel(
  ...
  const rateLimited = await checkRateLimit(sb, payload.companyId, channel);
  if (rateLimited) {
...
function buildDigestFromBatch(
  batch: { id: number; company_id: number; notification_type: string; channel: string },
  ...
  const typeMap: Record<string, { title: string; bodyFn: (n: number) => string }> = {
    raid_alert: {
      title: `Your planet was raided ${count} time${count > 1 ? "s" : ""}!`,
    ...
    kudos_received: {
      ...
      bodyFn: (n) => `${n} company${n > 1 ? "s" : ""} gave you kudos.`,
    },
  };
  ...
  const eventListHtml = items
    ...
    .map((item) => {
      ...
      return `<li style="margin-bottom: 4px; color: #f0f0f0;">${escapeBasicHtml(d.body || d.title || "New event")}</li>`;
    })
    .join("");
...
async function dispatchEmail(
  ...
  const email = await getcompanyEmail(payload.companyId);
  if (!email) {
  ...
  const unsubUrl = buildUnsubscribeUrl(payload.companyId, unsubCategory);
...
async function dispatchPush(
  ...
  if (!payload.forceSend) {
    const inQuietHours = await isInQuietHours(sb, payload.companyId);
    if (inQuietHours) {
...
async function dispatchInApp(
  ...
  await sb.from("notification_log").insert({
    ...
    channel: "in_app",
    notification_type: payload.type,
    recipient: String(payload.companyId),
    title: payload.title,
    status: "sent",
    metadata: {
      body: payload.body,
