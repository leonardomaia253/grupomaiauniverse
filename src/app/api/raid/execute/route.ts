...
export async function POST(request: Request) {
  ...
  if (raidError?.message?.includes("execute_raid")) {
    ...
    if (success) {
      ...
      admin.rpc("grant_xp", { p_company_id: attacker.id, p_source: "raid_win", p_amount: 50 }).then();
      admin.rpc("grant_xp", { p_company_id: defender.id, p_source: "raid_defend", p_amount: 30 }).then();
    } else {
      ...
      admin.rpc("grant_xp", { p_company_id: attacker.id, p_source: "raid_loss", p_amount: 15 }).then();
      admin.rpc("grant_xp", { p_company_id: defender.id, p_source: "raid_defend", p_amount: 30 }).then();
    }
    ...
    // Build planet position approximations (will be overridden client-side)
