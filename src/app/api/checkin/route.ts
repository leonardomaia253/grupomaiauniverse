...
async function grantStreakReward(
  ...
  for (const tier of [...STREAK_MILESTONES].reverse()) {
    ...
    await sb.from("purchases").insert({
      ...
      item_id: itemId,
      provider: "free",
      provider_tx_id: `streak_reward_${tier.milestone}_${companyId}`,
      amount_cents: 0,
      currency: "usd",
      status: "completed",
    });
...
export async function POST() {
  ...
  if (checkinResult.checked_in) {
    const { data: xpData } = await sb.rpc("grant_xp", { p_company_id: dev.id, p_source: "checkin", p_amount: 10 });
  ...
  if (checkinResult.checked_in) {
    ...
    if (checkinResult.streak >= 30 && !dev.streak_freeze_30d_claimed) {
      await sb.rpc("grant_streak_freeze", { p_company_id: dev.id });
      await sb
        .from("companies")
