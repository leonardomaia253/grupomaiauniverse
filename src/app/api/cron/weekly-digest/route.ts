  ...
  while (true) {
    ...
    if (achievementsResult.status === "fulfilled" && achievementsResult.value.data) {
      for (const a of achievementsResult.value.data) {
        achievementsMap.set(a.company_id, (achievementsMap.get(a.company_id) ?? 0) + 1);
      }
    }
    ...
    const sendResults = await Promise.allSettled(
      companies.map((dev) => {
        ...
        // Skip companies with zero activity
        ...
        if (dev.rank) {
          stats.push({ label: "Universe rank", value: `#${dev.rank}` });
        }
        ...
        sendNotificationAsync({
          ...
          title: `Your week in Mapa Vivo: ${dev.app_streak ?? 0}-day streak, rank #${dev.rank ?? "?"}`,
          body: `Streak: ${dev.app_streak ?? 0} days. Kudos: ${weeklyKudos}. Check your weekly recap.`,
          html: `
            <p style="color: #c8e64a; font-size: 16px;">Your week in Mapa Vivo</p>
            ${buildStatsTable(stats)}
            ${buildButton("Abrir Mapa Vivo", `${BASE_URL}/?user=${dev.username}`)}
          `,
