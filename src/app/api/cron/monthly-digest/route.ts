...
export async function GET(request: NextRequest) {
  ...
  while (true) {
    const { data: companies } = await sb
      .from("companies")
      .select("id, username, contributions, app_streak, rank")
      .eq("claimed", true)
      .not("email", "is", null)
    ...
    if (achievementsResult.status === "fulfilled" && achievementsResult.value.data) {
      for (const a of achievementsResult.value.data) {
        achievementCounts.set(a.company_id, (achievementCounts.get(a.company_id) ?? 0) + 1);
      }
    }
    ...
    const sendResults = await Promise.allSettled(
      companies.map((dev) => {
        ...
        if (dev.rank) stats.push({ label: "Universe rank", value: `#${dev.rank}` });
        ...
        sendNotificationAsync({
          ...
          title: `Your ${monthName} in Mapa Vivo`,
          body: `${monthName} recap: ${dev.contributions.toLocaleString()} total contributions, rank #${dev.rank ?? "?"}.`,
          html: `
            <p style="color: #c8e64a; font-size: 16px;">Your ${monthName} in Mapa Vivo</p>
