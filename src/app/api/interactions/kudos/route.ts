...
export async function POST(request: Request) {
  ...
  // Fetch giver (must have claimed planet)
  ...
  if (!insertError) {
    ...
    admin.rpc("grant_xp", { p_company_id: giver.id, p_source: "kudos_given", p_amount: 3 }).then();
    admin.rpc("grant_xp", { p_company_id: receiver.id, p_source: "kudos_received", p_amount: 1 }).then();
