...
export async function POST(request: Request) {
  ...
  // Validate user has claimed planet
  ...
  if (!dev || !dev.claimed) {
    return NextResponse.json(
      { error: "You must claim your planet first" },
      { status: 403 }
    );
  }
  ...
  if (dev.claimed_by !== user.id) {
    return NextResponse.json(
      { error: "This planet is not yours" },
      { status: 403 }
    );
  }
  ...
  if (gifted_to_login) {
    ...
    if (!receiver) {
      return NextResponse.json({ error: "User not found in Git Universe" }, { status: 400 });
    }
  ...
  // A11: Check scarUniverse constraints (temporal + quantity)
