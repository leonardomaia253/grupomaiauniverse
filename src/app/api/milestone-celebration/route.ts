...
export async function POST(req: Request) {
  const { total_companies } = await req.json().catch(() => ({ total_companies: 0 }));
  if (!total_companies || typeof total_companies !== "number") {
    return NextResponse.json({ error: "missing total_companies" }, { status: 400 });
  }

  // Find the highest milestone that's been crossed
  const milestone = [...MILESTONES].reverse().find((m) => total_companies >= m);
  if (!milestone) {
