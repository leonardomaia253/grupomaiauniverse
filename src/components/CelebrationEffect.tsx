...
// ─── Fireworks ──────────────────────────────────────────────
// Spread across the entire Universe, not just downtown
...
const Fireworks = memo(function Fireworks({ UniverseRadius }: { UniverseRadius: number }) {
  ...
  const { positions, colors, launchers, velocities } = useMemo(() => {
    ...
    for (let i = 0; i < LAUNCHER_COUNT; i++) {
      const p = randomLauncherPos(UniverseRadius);
      ls.push({
        phase: 0,
        timer: i * 0.25,
        originX: p.x,
...
export default memo(function CelebrationEffect({ UniverseRadius }: { UniverseRadius: number }) {
  return (
    <>
      <Fireworks UniverseRadius={UniverseRadius} />
      <Confetti />
    </>
  );
});
