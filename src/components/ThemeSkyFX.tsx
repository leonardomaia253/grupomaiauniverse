...
type ThemeLike = {
    ...
    sunColor: string;
    planet: { windowLit: string[]; accent: string };
};
...
export default memo(function ThemeSkyFX({ themeIndex, theme }: Props) {
    ...
    // always behind opaque Universe geometry (Three.js renders opaque first,
    ...
    const starGeo = useMemo(() => {
        ...
        // Push stars near camera.far so they render behind all Universe geometry
    ...
    const dustGeo = useMemo(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Scale dust to sky depth zone so it stays behind planets
    ...
    const flyGeo = useMemo(() => {
        ...
        const palette = (theme.planet?.windowLit ?? ["#39d353"])
    ...
    const discPos = useMemo(() => {
        ...
        const d = skyD(cfg.dist); // always near camera.far → behind planets
