...
function formatEvent(e: FeedEvent): string {
  ...
  switch (e.event_type) {
    ...
    case "planet_claimed":
      return `\u{1F3D7} ${actor} claimed their planet`;
    case "item_purchased":
    ...
    case "referral":
      return `\u{1F91D} ${meta.referrer_login ? `@${meta.referrer_login}` : actor} brought ${meta.referred_login ? `@${meta.referred_login}` : target} to the Universe`;
    case "gift_sent":
    ...
    case "dev_joined":
      return `\u{1F3D7} ${meta.login ? `@${meta.login}` : actor} joined the Universe`;
    case "visit_milestone":
      return `\u{1F440} ${meta.login ? `@${meta.login}` : target}'s planet got ${meta.visit_count ?? "many"} visits today`;
    case "item_equipped":
    ...
    case "raid_success":
      return `\u{1F4A5} ${meta.attacker_login ? `@${meta.attacker_login}` : actor} battled ${meta.defender_login ? `@${meta.defender_login}` : target}'s planet`;
    case "raid_failed":
    ...
    case "universe_star_verified":
      return `\u2B50 ${meta.login ? `@${meta.login}` : actor} unlocked the Estrela do Portfolio`;
    case "dev_highlight": {
      const login = meta.login ? `@${meta.login}` : actor;
      switch (meta.highlight) {
        case "contributions":
          return `#  ${login}'s planet has ${Number(meta.value).toLocaleString()} contributions`;
        case "stars":
          return `*  ${login} has ${Number(meta.value).toLocaleString()} stars across their repos`;
        case "rank":
          return `>>  ${login} is ranked #${meta.value} in the Universe`;
        case "streak":
        ...
        default:
          return `${login} is in the Universe`;
      }
    }
    default:
