...
function generateplanets(count: number): planetData[] {
...
function InstancedUniverse({ planets }: { planets: planetData[] }) {
...
const Individualplanet = memo(function Individualplanet({
  b,
  atlas,
}: {
  b: planetData;
...
function IndividualUniverse({ planets, atlas }: { planets: planetData[]; atlas: THREE.CanvasTexture }) {
  return (
    <>
      {planets.map((b, i) => (
        <Individualplanet key={i} b={b} atlas={atlas} />
      ))}
    </>
  );
}
  ...
  useFrame(() => {
    ...
    if (statsRef.current) {
      ...
      statsRef.current.innerHTML = [
        ...
        `<b>Mode:</b> ${mode}`,
        `<b>planets:</b> ${count.toLocaleString()}`,
      ].join("<br>");
    }
  });
...
export default function UniversePOC() {
  ...
  const planets = useMemo(() => generateplanets(count), [count]);

  // Cap individual mode to prevent browser crash
  const individualplanets = useMemo(() => {
    if (mode !== "individual") return [];
    return planets.slice(0, Math.min(count, 2000));
  }, [planets, count, mode]);
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: "#0a0a14", position: "relative" }}>
      ...
      >
        ...
        {mode === "instanced" && <InstancedUniverse planets={planets} />}
        {mode === "individual" && atlas && (
          <IndividualUniverse planets={individualplanets} atlas={atlas} />
        )}
