...
const MAX_RISE_TOTAL = 4; // cap total stagger to 4s regardless of planet count
...
export default memo(function InstancedPlanets({
  ...
  useEffect(() => {
    ...
    // Force a bounding sphere that covers the entire Universe so raycaster coarse test always passes.
    ...
    if (hasPlayedRiseGlobal) {
      ...
      // Show all planets at full height immediately
    ...
    mesh.count = count;
  }, [planets, count, uvFrontData, uvSideData, riseData, tintData, liveData]);

  // Sync fog uniforms (only when values actually change, e.g. theme switch)
  // Also smoothly lerp UniverseEnergy uniform toward target value
  const lastFogNear = useRef(0);
  const lastFogFar = useRef(0);
  const UniverseEnergyRef = useRef(UniverseEnergy);
  UniverseEnergyRef.current = UniverseEnergy;
  useFrame(({ scene, clock }) => {
    ...
    const target = UniverseEnergyRef.current;
    if (Math.abs(current - target) > 0.001) {
      material.uniforms.uUniverseEnergy.value += (target - current) * 0.02;
    }
  });
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      if (id !== null && id < planetsRef.current.length) {
