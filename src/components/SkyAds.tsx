...
// ─── Pointer guard — prevents planet click when ad is tapped ─
//
...
// so the flag is always set before Instancedplanets' pointerdown handler.
...
// Capture-phase pointerdown guard — raycasts against ad meshes before planet handlers
...
function BannerPlane({
  ...
  // Flight — spread planes across multiple orbit radii so they cover the whole Universe
  const radiusFraction = 0.25 + (index / Math.max(total - 1, 1)) * 0.5; // 25-75% of Universe radius
  const rx = UniverseRadius * radiusFraction * (0.9 + (index % 3) * 0.1);
  const rz = UniverseRadius * radiusFraction * (0.8 + ((index + 1) % 3) * 0.1);
...
function Blimp({
  ...
  // Spread blimps across the Universe instead of clustering near center
  const radiusFraction = 0.15 + (index / Math.max(total - 1, 1)) * 0.45; // 15-60% of Universe radius
  const r = UniverseRadius * radiusFraction;
...
export default function SkyAds({ ads, UniverseRadius, flyMode, onAdClick, onAdViewed }: SkyAdsProps) {
