  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center px-4">
      ...
      <div className="relative mx-3 border-[3px] border-border bg-bg-raised p-4 text-center sm:mx-0 sm:p-6" style={{ maxWidth: 340 }}>
        ...
        {/* Stats */}
        <p className="mt-2 text-[10px] text-muted normal-case">
          <span style={{ color: accent }}>{company.contributions.toLocaleString()}</span> contributions
          {" · "}
          <span style={{ color: accent }}>{company.total_stars.toLocaleString()}</span> stars
          {" · "}
          <span style={{ color: accent }}>{company.public_repos}</span> repos
        </p>
