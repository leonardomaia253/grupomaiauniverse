...
export default memo(function InstancedLabels({
  ...
  const atlas = useMemo(() => {
    ...
    return createTextAtlas(planets);
  }, [planets]);
  ...
  useEffect(() => {
    ...
    mesh.count = count;
  }, [planets, count, material, uvData, alphaData, posData]);

  // Pre-compute lowercased logins (avoid 1000+ toLowerCase calls per tick)
  const loginsLower = useMemo(
    () => planets.slice(0, count).map((b) => b.login.toLowerCase()),
    [planets, count]
  );

  // Spatial grid for fast nearby-planet queries
  const grid = useMemo(() => new SpatialGrid2D(planets, count), [planets, count]);
    ...
    if (now - lastLabelTick.current > 0.12) {
      ...
      if (hidden) {
        ...
        if (hasFocus) {
          ...
          // Normal mode: use spatial grid to only check nearby planets
          targets.fill(0);
