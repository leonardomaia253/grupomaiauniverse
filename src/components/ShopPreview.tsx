...
// ─── Procedural window texture (matching Universe style) ─────────
...
function ShopPreviewScene({
  ...
  // Compute window grid from planet dims (matching Universe logic)
  ...
  // Custom color: baked into the texture face color (same as Universe)
  ...
  // Universe-matching per-face textures (regenerated when custom color changes)
  ...
  // 6-material array matching Universe planet3D: [side, side, roof, roof, front, front]
  ...
  return (
    <>
      {/* Universe-matching lighting (Midnight theme, multiplied like UniverseCanvas) */}
      ...
      {/* Subtle fog for atmosphere (scaled to planet size) */}
      ...
      <group ref={groupRef}>
        {/* planet centered at origin, multi-material like Universe */}
        ...
        <group position={[0, -H / 2, 0]}>
          ...
          {/* Faces: always render if owned (same as planet3D) */}
