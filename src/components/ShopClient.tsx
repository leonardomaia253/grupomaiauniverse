  ...
  return (
    <div className="mt-2 border-2 border-border/50 bg-bg/50 px-3 py-2">
      {isOwned ? (
        <>
          ...
          {!autoUploading && images.filter(Boolean).length === 0 && (
            <div className="mb-2 border-2 border-dashed px-3 py-2 text-[10px] normal-case" style={{ borderColor: ACCENT, color: ACCENT }}>
              Upload an image to each slot below to display on your planet!
            </div>
          )}
        ...
        <>
          <p className="mb-2 text-[9px] text-muted normal-case">
            Try it — pick an image to preview on the 3D planet. Purchase to save.
          </p>
...
export default function ShopClient({
  ...
  const [activeTab, setActiveTab] = useState<"planet" | "raid">(() => {
  ...
  useEffect(() => {
    try {
      ...
      // Brazilian IANA timezones all use Brazilian Universe names
