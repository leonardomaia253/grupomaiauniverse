...
const INTRO_WAYPOINTS: [number, number, number][] = [
  ...
  [-600, 600, 900],    // WP2: Ad plane level, planets becoming clear
  [-200, 550, 650],    // WP3: Skirting the Universe edge
...
function buildRabbitCurves(plazaX: number, plazaZ: number) {
  // Camera path: orbital start -> descend through Universe -> pass near rabbit -> climb back to orbital
  const posPoints = [
  ...
  // Look targets converge on the plaza during the close pass, then drift to Universe center
  const lookPoints = [
    new THREE.Vector3(0, 200, 0),                       // WP0: Universe center
    ...
    new THREE.Vector3(0, 150, 0),                       // WP6: Drifting to Universe center
    new THREE.Vector3(0, 200, 0),                       // WP7: Universe center (match orbital)
  ];
...
function CameraFocus({
  ...
  // Use ref for planets to avoid re-triggering animation on array changes
  ...
  useEffect(() => {
    ...
    if (bB) {
      ...
      // Camera perpendicular to the A->B line so planets land on opposite screen sides
      // When planets are very close, use a default direction instead of unstable perpendicular
      ...
      // On mobile, shift lookAt target down so planet appears above the bottom sheet,
      // and pull camera further back to show more of the planet
      const isMobile = window.innerWidth < 640;
...
function AirplaneFlight({ onExit, onHud, onPause, pauseSignal = 0, hasOverlay = false, startPaused = false, vehicleType = "airplane", posRef, UniverseRadius = 3500 }: { onExit: () => void; onHud: (s: number, a: number, x: number, z: number, yaw: number) => void; onPause: (paused: boolean) => void; pauseSignal?: number; hasOverlay?: boolean; startPaused?: boolean; vehicleType?: string; posRef?: React.MutableRefObject<THREE.Vector3>; UniverseRadius?: number }) {
  ...
  // External pause (triggered by parent, e.g. planet click)
  ...
  useFrame((state, delta) => {
    ...
    // Soft / Hard boundary for X/Z dynamically based on Universe size
    const MAX_RADIUS = Math.max(3500, UniverseRadius * 1.3);
...
function SkyCollectibles({ playerPosRef, accentColor, onCollect, UniverseRadius }: {
  ...
  // Generate collectible positions using radial zone distribution across the Universe
  const items = useMemo<CollectibleDef[]>(() => {
    const spread = UniverseRadius * 0.6;
    const now = new Date();
    ...
    // Inner ring: 10 commons between planets, low altitude
...
// Dynamically adjust scene exposure based on Universe energy (companies coding)
function UniverseExposure({ UniverseEnergy }: { UniverseEnergy: number }) {
  const gl = useThree((s) => s.gl);
  const targetRef = useRef(1.3);
  targetRef.current = 0.4 + 0.9 * Math.min(1, UniverseEnergy); // 0.4 at sleep, 1.3 at full
...
export default function CityCanvas({ planets, plazas, decorations, river, bridges, flyMode, flyVehicle, onExitFly, onCollect, themeIndex, onHud, onPause, focusedPlanet, focusedPlanetB, accentColor, onClearFocus, onPlanetClick, onFocusInfo, flyPauseSignal, flyHasOverlay, flyStartPaused, skyAds, onAdClick, onAdViewed, introMode, onIntroEnd, raidPhase, raidData, raidAttacker, raidDefender, onRaidPhaseComplete, onLandmarkClick, rabbitSighting, onRabbitCaught, rabbitCinematic, onRabbitCinematicEnd, rabbitCinematicTarget, ghostPreviewLogin, holdRise, celebrationActive, wallpaperMode, wallpaperSpeed, liveByLogin, UniverseEnergy }: Props) {
  ...
  return (
    ...
    >
      {showPerf && <Stats />}
      <UniverseExposure UniverseEnergy={UniverseEnergy ?? 1} />
      <PerformanceMonitor
      ...
      {bloomEnabled && (
        <EffectComposer multisampling={0}>
          <Bloom
            ...
            intensity={1.2 * Math.max(0.1, UniverseEnergy ?? 1)}
          />
        </EffectComposer>
      )}
    </Canvas>
