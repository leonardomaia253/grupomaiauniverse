...
export default function constellationChooser({
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch("/api/constellation/change", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ constellation_id: selected }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-lg overflow-y-auto border-[3px] border-border bg-bg-raised p-5 sm:p-6">
        ...
        <h2 className="text-lg text-cream">Choose your constellation</h2>
        ...
        >
          {submitting ? "Confirming..." : "Confirm constellation"}
        </button>
      </div>
    </div>
  );
