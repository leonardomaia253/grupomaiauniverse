...
//
// Wide rectangular panel bolted to the planet face.
...
function AdBillboard({
  ...
  const { width, depth, height } = planet;
  ...
  return (
    <group position={[planet.position[0], 0, planet.position[2]]}>
...
//
// Thin horizontal strip (2 units tall) that hugs the planet on all sides.
...
function AdLedWrap({
  ...
  const y = height * 0.88;
  const gap = 0.15; // offset from planet face
  ...
  // Invisible viewability proxy at planet center — covers all 4 directions
...
export default function planetAds({ ads, planets, onAdClick, onAdViewed, focusedplanet, focusedplanetB }: planetAdsProps) {
  ...
  // Distance culling: hide all planet ads when camera is far away
  useFrame(() => {
  ...
  return (
    <group ref={groupRef}>
      {billboardAds.map((ad, i) => {
        const planet = top10[i];
        if (!planet) return null;
        const loginLower = planet.login.toLowerCase();
