/**
 * Indexes planets for auth users who logged in but were never fetched from GitHub.
 *
 * Usage:
 *   npx tsx scripts/index-missing-companies.ts
 *   npx tsx scripts/index-missing-companies.ts --dry-run
 *   npx tsx scripts/index-missing-companies.ts --concurrency=3
 */
...
const BASE_URL = process.env.NEXT_PUBLIC_APP_URL || "https://universe.grupomaia.me";
...
async function getMissingLogins(): Promise<string[]> {
  ...
  while (true) {
    const { data, error } = await sb
      .rpc("get_auth_users_without_company")
...
async function main() {
  console.log("Fetching auth users without a company record...");
  const missing = await getMissingLogins();
  console.log(`Found ${missing.length} missing companies.`);
