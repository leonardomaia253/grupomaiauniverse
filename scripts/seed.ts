...
async function fetchAndUpsert(login: string): Promise<boolean> {
  try {
    ...
    const ghCompany = await userRes.json();

    if (ghCompany.type === "Organization") {
    ...
    const [expanded, reposPage1Res] = await Promise.all([
      fetchExpandedGitHubData(ghCompany.login),
      fetch(
    ...
    const { error } = await sb.from("companies").upsert(
      {
        github_login: ghCompany.login.toLowerCase(),
        github_id: ghCompany.id,
        name: ghCompany.name,
        avatar_url: ghCompany.avatar_url,
        bio: ghCompany.bio,
        contributions,
        public_repos: ghCompany.public_repos,
    ...
    console.log(
      `  [OK]   ${ghCompany.login} — ${contributions} contribs (total: ${expanded?.contributions_total ?? '?'}), ${totalStars} stars, ${repos.length} repos`
    );
    return true;
  } catch (err) {
...
async function main() {
  console.log(`\nSeeding Git Universe with ${TOP_COMPANIES.length} companies...\n`);
