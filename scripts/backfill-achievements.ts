...
async function main() {
  ...
  // 2. Load all companies with stats
  const { data: companies, error: devErr } = await sb
    .from("companies")
    .select("id, github_login, contributions, public_repos, total_stars, kudos_count, referral_count")
    .order("rank");

  if (devErr || !companies) {
    console.error("Failed to load companies:", devErr);
    return;
  }
  ...
  const existingSet = new Set(
    (existing ?? []).map((r) => `${r.company_id}_${r.achievement_id}`)
  );
  ...
  for (const r of giftsSent ?? []) {
    giftsSentMap[r.company_id] = (giftsSentMap[r.company_id] ?? 0) + 1;
  }
  ...
  // 5. Process each dev
  let totalUnlocks = 0;
  const unlockRows: { company_id: number; achievement_id: string }[] = [];
  const purchaseRows: {
    company_id: number;
    item_id: string;
    provider: string;
  ...
  for (const dev of companies) {
    ...
    if (newUnlocks.length > 0) {
      ...
      for (const a of newUnlocks) {
        unlockRows.push({ company_id: dev.id, achievement_id: a.id });
  ...
  // 6. Batch insert
  console.log("\nInserting company_achievements...");
