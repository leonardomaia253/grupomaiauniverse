...
async function main() {
  ...
  const kudos = await fetchAll("company_kudos", "select=giver_id,receiver_id,given_date,created_at&order=created_at.asc");
  const kudosByDay = groupByDay(kudos, "created_at");

  // 6. planet visits
  console.log("  Puxando planet visits...");
  const visits = await fetchAll("planet_visits", "select=visitor_id,planet_id,visit_date,created_at&order=created_at.asc");
  const visitsByDay = groupByDay(visits, "created_at");

  // 7. Achievements unlocked
  console.log("  Puxando achievements...");
  const achievements = await fetchAll("company_achievements", "select=company_id,achievement_id,unlocked_at&order=unlocked_at.asc");
  const achievementsByDay = groupByDay(achievements, "unlocked_at");

  // 8. Streak checkins
  console.log("  Puxando streak checkins...");
  const checkins = await fetchAll("streak_checkins", "select=company_id,checkin_date,type&order=checkin_date.asc");
  const checkinsByDay = groupByDay(checkins, "checkin_date");

  // 9. Universe stats
  console.log("  Puxando Universe stats...");
  const { data: UniverseStats } = await query("Universe_stats", "id=eq.1");
  const stats = UniverseStats[0] || {};
  ...
  // Top companies by kudos
  const topKudos = [...companies].sort((a, b) => b.kudos_count - a.kudos_count).slice(0, 10);

  // Top companies by visits
  const topVisits = [...companies].sort((a, b) => b.visit_count - a.visit_count).slice(0, 10);

  // Top companies by contributions
  const topContribs = [...companies].sort((a, b) => b.contributions - a.contributions).slice(0, 10);

  // Top companies by referrals
  const topReferrals = [...companies].filter((d) => d.referral_count > 0).sort((a, b) => b.referral_count - a.referral_count).slice(0, 10);
  ...
  const claimedcompanies = companies.filter((d) => d.claimed);
  const unclaimedcompanies = companies.filter((d) => !d.claimed);
  const claimedByDay = groupByDay(claimedcompanies.filter((d) => d.claimed_at), "claimed_at");
  const claimRate = ((claimedcompanies.length / companies.length) * 100).toFixed(1);
  ...
  // Aggregate stats
  const totalStars = companies.reduce((s, d) => s + (d.total_stars || 0), 0);
  const totalRepos = companies.reduce((s, d) => s + (d.public_repos || 0), 0);
  const totalPRs = companies.reduce((s, d) => s + (d.total_prs || 0), 0);
  const totalReviews = companies.reduce((s, d) => s + (d.total_reviews || 0), 0);
  const totalIssues = companies.reduce((s, d) => s + (d.total_issues || 0), 0);
  const totalFollowers = companies.reduce((s, d) => s + (d.followers || 0), 0);
  const totalContribsAll = companies.reduce((s, d) => s + (d.contributions_total || d.contributions || 0), 0);

  // Notable companies (top by followers or stars)
  const topByFollowers = [...companies].sort((a, b) => (b.followers || 0) - (a.followers || 0)).slice(0, 20);
  const topByStars = [...companies].sort((a, b) => (b.total_stars || 0) - (a.total_stars || 0)).slice(0, 20);
  ...
  // Active streaks
  const activeStreaks = companies.filter((d) => d.app_streak > 0);
  const longestStreak = Math.max(...companies.map((d) => d.app_longest_streak || 0));
  ...
  const oldestAccounts = companies.filter((d) => d.account_created_at).sort((a, b) => new Date(a.account_created_at) - new Date(b.account_created_at)).slice(0, 10);
  const newestAccounts = companies.filter((d) => d.account_created_at).sort((a, b) => new Date(b.account_created_at) - new Date(a.account_created_at)).slice(0, 5);

  // Day-over-day growth
  const growthRates = [];
  let prevCum = 0;
  for (const day of [...new Set(companiesByDay.map(([d]) => d))].sort()) {
    const count = Object.fromEntries(companiesByDay)[day] || 0;
  ...
  for (const [day] of companiesByDay) allDays.add(day);
  ...
  const companiesMap = dayMap(companiesByDay);
  ...
  let report = `# Git Universe Analytics Report
...
| Metrica | Valor |
|---------|-------|
| Total de companies | ${formatNum(companies.length)} |
| companies claimed (logaram) | ${formatNum(claimedcompanies.length)} (${claimRate}%) |
| companies nao claimed | ${formatNum(unclaimedcompanies.length)} |
| Total de contribuicoes | ${formatNum(totalContribsAll)} |
| Total de repos publicos | ${formatNum(totalRepos)} |
| Total de stars (todos os companies) | ${formatNum(totalStars)} |
| Total de PRs | ${formatNum(totalPRs)} |
| Total de reviews | ${formatNum(totalReviews)} |
| Total de issues | ${formatNum(totalIssues)} |
| Total de followers (todos os companies) | ${formatNum(totalFollowers)} |
...
| planet visits | ${formatNum(visits.length)} |
...
| companies com streak ativo | ${formatNum(activeStreaks.length)} |
...
| Dia | Novos companies | Acumulado | Compras | Ads pagos | Kudos | Visits | Checkins | Achievements |
|-----|-----------|-----------|---------|-----------|-------|--------|----------|-------------|
`;

  let cumcompanies = 0;
  for (const day of sortedDays) {
    const nd = companiesMap[day] || 0;
    cumcompanies += nd;
    ...
    report += `| ${day} | ${nd} | ${formatNum(cumcompanies)} | ${p} | ${a} | ${k} | ${v} | ${c} | ${ach} |\n`;
  }
...
`;
  ...
  report += `
## Top 10 companies por Contribuicoes
...
`;
  ...
  // Notable companies by followers
  report += `
---

## companies mais famosos na cidade (por followers)
...
`;
  ...
  // Notable companies by stars
  report += `
## companies com mais stars na cidade
...
`;
  ...
  // Oldest GitHub accounts in the Universe
  report += `
...
| companies adicionados | ${formatNum(companies.length)} | ${((companies.length / 15000) * 100).toFixed(1)}% |
| companies claimed (logaram) | ${formatNum(claimedcompanies.length)} | ${((claimedcompanies.length / 15000) * 100).toFixed(1)}% |
...
- ${formatNum(companies.length)} companies represented in the Universe em ${sortedDays.length} dias
- ${formatNum(claimedcompanies.length)} companies logged in and claimed their planets
- ${formatNum(totalStars)} combined GitHub stars across all companies
...
- Built by a solo company in 1 week

### Crescimento diario medio
- ${(companies.length / Math.max(sortedDays.length, 1)).toFixed(0)} novos companies/dia
- ${(kudos.length / Math.max(sortedDays.length, 1)).toFixed(0)} kudos/dia
- ${(visits.length / Math.max(sortedDays.length, 1)).toFixed(0)} planet visits/dia

### Dados que impressionam investidores/parceiros
- Maior dia: ${Math.max(...Object.values(Object.fromEntries(companiesByDay)))} companies em 24h
- Taxa de claim: ${claimRate}% (companies que logaram vs adicionados)
...
`;
  ...
  console.log(`\nResumo rapido:`);
  console.log(`  companies: ${formatNum(companies.length)}`);
